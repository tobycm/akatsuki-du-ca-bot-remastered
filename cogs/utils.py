...
class UtilsCog(Cog):
    ...
    async def server_info(self, itr: Interaction):
        ...
        embed.add_field(name="Server Age", value=f"<r:{guild.created_at.timestamp()}:D")
    ...
    async def user_info(self, itr: Interaction, user: Optional[Member] = None):
        ...
        embed.add_field(name="User Joined", value=f"<r:{user.joined_at.timestamp()}:D")
        embed.add_field(name="User Creation Date", value=f"<r:{user.created_at.timestamp()}:D")
